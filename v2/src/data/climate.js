...
//
// Урок от Кристиансанд: градът изглеждаше добре по слънце — южно
// крайбрежие, по-слънчево от останалата Норвегия — и лош по единственото,
// което не бяхме погледнали: вятъра. Затова климатът тук не е едно число.
//
...
//
// `dark` вероятно тежи повече от слънчевите часове за семейство с деца:
// три месеца по шест часа светлина се усещат всеки ден.
//
// ВСИЧКИ СТОЙНОСТИ СА ОЦЕНКИ. Не са проверени за 2026 и не идват от
// метеорологична служба, освен където изрично е отбелязано друго.
...
export const CLIMATE = {
  ...
  // Проверено частично: средна скорост на вятъра варира силно по източник
  // (13–24 км/ч в различни таблици), но максимум 88.9 км/ч е регистриран и
  // зимите последователно се описват като "windy" в независими източници.
  // Южно изложение → по-слънчево от останала Норвегия, но именно затова
  // вятърът е пропуснатият фактор, не компенсираният. storm тук е висок
  // нарочно, за да пресъздаде наблюдението, което изключи града.
  kristiansand:{wind:5.2,storm:60,rain:185,jan:0.5,dark:6.3},
